...
function AnimatedStat({ value, label, icon, color, delay = 0 }) {
  ...
  return (
    <div ref={ref} className={`stat-card reveal ${vis ? 'shown' : ''}`} style={{ '--accent': color, transitionDelay: `${delay}ms` }}>
      ...
      <div className="stat-value" style={{ color }}>{count.toLocaleString()}{value.replace(/[0-9]/g, '')}</div>
      ...
      <div className="ab-page">
        ...
        <section ref={heroRef} className="ab-hero">
          <div className="ab-hero-bg">
            <div className="ab-hero-orb" style={{ top: '-20%', left: '-10%', background: `radial-gradient(circle, rgba(13,148,136,.08), transparent)` }} />
            <div className="ab-hero-orb" style={{ bottom: '-10%', right: '-5%', background: `radial-gradient(circle, rgba(249,115,22,.06), transparent)` }} />
          </div>
          ...
          <div className={`ab-hero-content reveal ${heroVis ? 'shown' : ''}`}>
            ...
            <p className="ab-hero-subtitle">
              CediMart connects thousands of students across Ghana's universities — making it easy to buy, sell, and grow together.
            </p>
            ...
            <div className={`ab-mission-grid reveal ${missionVis ? 'shown' : ''}`}>
              <div className="ab-mission-text">
                ...
                <h2 className="ab-section-title">
                  ...
                  <span style={{ color: C.brand }}> safe, simple, and social.</span>
                </h2>
                <p className="ab-section-desc">
                  We believe every student should have access to affordable goods and the opportunity to earn. CediMart removes the friction from campus buying and selling.
                </p>
                <p className="ab-section-desc">
        ...
        <section className="ab-stats-section">
          <div className="ab-section-inner">
            <div className="ab-stats-grid">
              <AnimatedStat value="10000+" label="Active Students" icon="👥" color={C.brandL} delay={0} />
              <AnimatedStat value="2500+" label="Verified Businesses" icon="🏪" color={C.success} delay={100} />
              <AnimatedStat value="50000+" label="Total Listings" icon="📦" color={C.accent} delay={200} />
              <AnimatedStat value="8" label="Campuses" icon="🏫" color={C.danger} delay={300} />
            </div>
          </div>
        </section>
        ...
        <section ref={storyRef} className="ab-section">
          <div className="ab-section-inner">
            <div className={`ab-story-header reveal ${storyVis ? 'shown' : ''}`}>
              ...
              <h2 className="ab-section-title">
                From a hostel room
                <span style={{ color: C.accent }}> to every campus.</span>
              </h2>
            </div>
            <div className="ab-timeline">
              {[
                { year: '2023', title: 'The Idea', description: 'Frustrated with scattered WhatsApp groups and unreliable notice boards, a group of students at University of Ghana decided to build a better way.', icon: '💡' },
                { year: 'Early 2024', title: 'First Launch at UG', description: 'CediMart launched at University of Ghana with 200 beta users. Within the first month, over 1,000 listings were posted.', icon: '🚀' },
                { year: 'Mid 2024', title: 'Expanding to More Campuses', description: 'KNUST, UCC, and UPSA joined the platform. We introduced verified seller badges and campus-specific filtering.', icon: '🏫' },
                { year: 'Late 2024', title: 'CediAI is Born', description: 'We launched our AI shopping assistant — the first of its kind for a student marketplace in Ghana.', icon: '🤖' },
                { year: '2025', title: '10,000+ Students Strong', description: 'CediMart hit 10,000 active students across 8 campuses with over 50,000 total listings.', icon: '🎉', isLast: true },
        ...
        <section ref={valuesRef} className="ab-section ab-values-section">
          <div className="ab-section-inner">
            <div className={`reveal ${valuesVis ? 'shown' : ''}`} style={{ textAlign: 'center', marginBottom: 48 }}>
              ...
              <h2 className="ab-section-title">What drives<span style={{ color: C.accent }}> everything we do.</span></h2>
            </div>
            <div className="ab-values-grid">
              {[
                { icon: '🔒', title: 'Trust & Transparency', color: C.brand, desc: 'Every transaction is protected. Every seller is verified. We believe trust is the foundation of any marketplace.' },
                { icon: '🌍', title: 'Made in Ghana', color: C.success, desc: 'Built by Ghanaian students, for Ghanaian students. We understand the unique needs of campus life in Ghana.' },
                { icon: '⚡', title: 'Speed & Innovation', color: C.accent, desc: 'From AI-powered search to instant listings, we leverage technology to make campus commerce effortless.' },
                { icon: '🤝', title: 'Community First', color: C.danger, desc: 'Every feature we build starts with a simple question: "Does this make campus life better for students?"' },
                { icon: '📈', title: 'Empowerment', color: C.brandL, desc: 'We give students the tools to earn income and build businesses while still in school.' },
                { icon: '♻️', title: 'Sustainability', color: '#14B8A6', desc: 'Buying and selling pre-owned items reduces waste and makes quality goods accessible to every student.' },
              ].map((value, i) => (
        ...
        <section ref={teamRef} className="ab-section">
          <div className="ab-section-inner">
            <div className={`reveal ${teamVis ? 'shown' : ''}`} style={{ textAlign: 'center', marginBottom: 48 }}>
              ...
              <h2 className="ab-section-title">Built by students,<span style={{ color: C.brandL }}> for students.</span></h2>
            </div>
            <div className="ab-team-grid">
              ...
              <TeamCard name="Campus Ambassadors" role="Community Managers" emoji="📣" color={C.accent} delay={160} />
...
const aboutStyles = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .ab-hero-subtitle { font-size: clamp(14px,2vw,17px); color: ${C.off}; line-height: 1.7; max-width: 600px; margin: 0 auto 40px; }
  ...
  .ab-cta-subtitle { font-size: 15px; color: rgba(255,255,255,.7); max-width: 450px; margin: 0 auto 28px; line-height: 1.6; position: relative; }
  ...
  .ab-cta-links a { color: rgba(255,255,255,.6); font-size: 13px; font-weight: 600; text-decoration: none; transition: color .2s; }

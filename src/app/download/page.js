      ...
      <div className="dl-page">
        ...
        <section ref={heroRef} className="dl-hero">
          <div className="dl-hero-bg">
            <div className="dl-hero-orb" style={{ top: '-10%', right: '-5%', background: `radial-gradient(circle, rgba(13,148,136,.08), transparent)` }} />
            <div className="dl-hero-orb" style={{ bottom: '-15%', left: '-5%', background: `radial-gradient(circle, rgba(249,115,22,.06), transparent)` }} />
        ...
        <section ref={stepsRef} className="dl-section dl-steps-section">
          <div className="dl-section-inner">
            <div className={`reveal ${stepsVis ? 'shown' : ''}`} style={{ textAlign: 'center', marginBottom: 48 }}>
              ...
              <h2 className="dl-section-title">
                Three simple steps
                <span style={{ color: C.accent }}> to get going.</span>
              </h2>
            </div>
        ...
        <section ref={faqRef} className="dl-section dl-faq-section">
          <div className="dl-section-inner">
            <div className={`reveal ${faqVis ? 'shown' : ''}`} style={{ textAlign: 'center', marginBottom: 36 }}>
              ...
              <h2 className="dl-section-title">
                Quick answers to
                <span style={{ color: C.brandL }}> common questions.</span>
              </h2>
            </div>
...
const downloadStyles = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .dl-page { background: ${C.void}; color: ${C.white}; font-family: 'Plus Jakarta Sans', sans-serif; overflow-x: hidden; }
  ...
  .dl-hero { position: relative; display: grid; grid-template-columns: 1fr 1fr; gap: clamp(32px,5vw,60px); align-items: center; padding: clamp(60px,10vw,120px) clamp(20px,5vw,80px); max-width: 1280px; margin: 0 auto; overflow: hidden; }
  @media (max-width: 900px) { .dl-hero { grid-template-columns: 1fr; text-align: center; } .dl-hero-phone { display: none !important; } }
  .dl-hero-bg { position: absolute; inset: 0; pointer-events: none; }
  .dl-hero-orb { position: absolute; width: 500px; height: 500px; border-radius: 50%; filter: blur(100px); }
  .dl-hero-grid-pattern { position: absolute; inset: 0; opacity: .03; background-image: radial-gradient(circle, #0F172A 1px, transparent 1px); background-size: 32px 32px; }
  .dl-hero-content { position: relative; z-index: 2; }
  .dl-hero-badge { display: inline-flex; align-items: center; gap: 8px; font-size: 11px; font-weight: 700; letter-spacing: .14em; text-transform: uppercase; color: ${C.brand}; background: ${C.brandBg}; border: 1px solid ${C.brandBorder}; border-radius: 40px; padding: 6px 16px; margin-bottom: 24px; }
  .dl-hero-badge-dot { width: 6px; height: 6px; border-radius: 50%; background: ${C.brand}; }
  .dl-hero-title { font-size: clamp(36px,5.5vw,72px); font-weight: 900; line-height: 1.05; letter-spacing: -2px; margin-bottom: 18px; }
  .dl-hero-highlight { display: block; background: linear-gradient(135deg, ${C.brand}, ${C.accent}); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }
  .dl-hero-subtitle { font-size: clamp(14px,2vw,17px); color: ${C.off}; line-height: 1.7; max-width: 480px; margin-bottom: 32px; }
  @media (max-width: 900px) { .dl-hero-subtitle { margin-left: auto; margin-right: auto; } }

  .dl-hero-btns { display: flex; gap: 12px; flex-wrap: wrap; margin-bottom: 36px; }
  @media (max-width: 900px) { .dl-hero-btns { justify-content: center; } }
  @media (max-width: 480px) { .dl-hero-btns { flex-direction: column; } }
  .dl-hero-btn { display: inline-flex; align-items: center; gap: 12px; padding: 14px 24px; border-radius: 16px; text-decoration: none; transition: all .25s; font-family: 'Plus Jakarta Sans', sans-serif; }
  .dl-btn-ios { background: #fff; color: #000; box-shadow: 0 8px 28px rgba(0,0,0,.1); }
  ...
  .dl-steps-section { background: ${C.surf}; border-top: 1px solid ${C.border}; border-bottom: 1px solid ${C.border}; }
  .dl-steps-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 24px; position: relative; }
  @media (max-width: 640px) { .dl-steps-grid { grid-template-columns: 1fr; max-width: 350px; margin: 0 auto; } }
  ...
  .dl-reviews-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)); gap: 16px; }
  ...
  .dl-faq-section { background: ${C.surf}; border-top: 1px solid ${C.border}; border-bottom: 1px solid ${C.border}; }
  .dl-faq-list { max-width: 650px; margin: 0 auto; display: flex; flex-direction: column; gap: 8px; }
  ...
  .dl-bottom-btn.white { background: #fff; color: ${C.brandD}; box-shadow: 0 8px 28px rgba(0,0,0,.1); }
  .dl-bottom-btn.white:hover { transform: translateY(-2px); box-shadow: 0 14px 36px rgba(0,0,0,.15); }
  .dl-bottom-btn.ghost { background: rgba(255,255,255,.12); color: #fff; border: 1.5px solid rgba(255,255,255,.3); }
  .dl-bottom-btn.ghost:hover { background: rgba(255,255,255,.2); }
  .dl-bottom-links { margin-top: 24px; position: relative; }
  .dl-bottom-links a { color: rgba(255,255,255,.6); font-size: 13px; font-weight: 600; text-decoration: none; transition: color .2s; }
